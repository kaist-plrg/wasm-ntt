...
let sl_to_fmagN (layout: layout) (value: Value.t) : Z.t =
  ...
  | Some ([[{ it = Atom "NORM"; _ }]; _; _], [m; exp]) -> Z.(shift_left (sl_to_z_int exp + bias layout) layout.mantissa + sl_to_z_nat m)
...
and sl_to_vec_extension (value: Value.t) : Pack.vec_extension =
  match sl_case value with
  | Some ([[{ it = Atom "ExtLane"; _ }]; _; _], [shape; ext]) -> Pack.ExtLane (sl_to_pack_shape shape, sl_to_extension ext)
...
and sl_to_catch' (value: Value.t) : Ast.catch' =
  match sl_case value with
  | Some ([[{ it = Atom "Catch"; _ }]; _; _], [idx1; idx2]) -> Catch (sl_to_idx idx1, sl_to_idx idx2)
  | Some ([[{ it = Atom "CatchRef"; _ }]; _; _], [idx1; idx2]) -> CatchRef (sl_to_idx idx1, sl_to_idx idx2)
...
and sl_to_vextractop (value: Value.t) : Ast.vec_extractop =
  ...
  | Some ([[{ it = Atom "V128"; _ }]; _], [vop]) ->
    (match sl_case vop with
    | Some ([[{ it = Atom "I8x16"; _ }]; _], [op]) -> Wasm_Value.V128 (V128.I8x16 (sl_to_int_vnextractop op))
    | Some ([[{ it = Atom "I16x8"; _ }]; _], [op]) -> Wasm_Value.V128 (V128.I16x8 (sl_to_int_vnextractop op))
    | Some ([[{ it = Atom "I32x4"; _ }]; _], [op]) -> Wasm_Value.V128 (V128.I32x4 (sl_to_float_vnextractop op))
    | Some ([[{ it = Atom "I64x2"; _ }]; _], [op]) -> Wasm_Value.V128 (V128.I64x2 (sl_to_float_vnextractop op))
    | Some ([[{ it = Atom "F32x4"; _ }]; _], [op]) -> Wasm_Value.V128 (V128.F32x4 (sl_to_float_vnextractop op))
    | Some ([[{ it = Atom "F64x2"; _ }]; _], [op]) -> Wasm_Value.V128 (V128.F64x2 (sl_to_float_vnextractop op))
...
and sl_to_vnreplaceop (value: Value.t) : Ast.V128Op.nreplaceop =
  match sl_case value with
  | Some ([[{ it = Atom "Replace"; _ }]; _], [i]) -> Replace (sl_to_int i)
...
and sl_to_instr' (value: Value.t) : Ast.instr' =
  ...
  | Some ([[{ it = Atom "BLOCK"; _ }]; _; _], [bt; instrs]) -> Block (sl_to_block_type bt, sl_to_list sl_to_instr instrs)
  | Some ([[{ it = Atom "LOOP"; _ }]; _; _], [bt; instrs]) -> Loop (sl_to_block_type bt, sl_to_list sl_to_instr instrs)
  ...
  | Some ([[{ it = Atom "BR_TABLE"; _ }]; _; _], [idxl; idx]) -> BrTable (sl_to_list sl_to_idx idxl, sl_to_idx idx)
  | Some ([[{ it = Atom "BR_ON_NULL"; _ }]; _], [idx]) -> BrOnNull (sl_to_idx idx)
  | Some ([[{ it = Atom "BR_ON_NON_NULL"; _ }]; _], [idx]) -> BrOnNonNull (sl_to_idx idx)
  | Some ([[{ it = Atom "BR_ON_CAST"; _ }]; _; _; _], [idx; rt1; rt2]) -> BrOnCast (sl_to_idx idx, sl_to_ref_type rt1, sl_to_ref_type rt2)
  | Some ([[{ it = Atom "BR_ON_CAST_FAIL"; _ }]; _; _; _], [idx; rt1; rt2]) -> BrOnCastFail (sl_to_idx idx, sl_to_ref_type rt1, sl_to_ref_type rt2)
  ...
  | Some ([[{ it = Atom "CALL_INDIRECT"; _ }]; _; _], [idx1; idx2]) -> CallIndirect (sl_to_idx idx1, sl_to_idx idx2)
  ...
  | Some ([[{ it = Atom "RETURN_CALL_INDIRECT"; _ }]; _; _], [idx1; idx2]) -> ReturnCallIndirect (sl_to_idx idx1, sl_to_idx idx2)
  | Some ([[{ it = Atom "THROW"; _ }]; _], [idx]) -> Throw (sl_to_idx idx)
  | Some ([[{ it = Atom "THROW_REF"; _ }]], []) -> ThrowRef
  | Some ([[{ it = Atom "TRY_TABLE"; _ }]; _; _; _], [bt; catches; instrs]) -> TryTable (sl_to_block_type bt, sl_to_list sl_to_catch catches, sl_to_list sl_to_instr instrs)
  ...
  | Some ([[{ it = Atom "TABLE.COPY"; _ }]; _; _], [idx1; idx2]) -> TableCopy (sl_to_idx idx1, sl_to_idx idx2)
  | Some ([[{ it = Atom "TABLE.INIT"; _ }]; _; _], [idx1; idx2]) -> TableInit (sl_to_idx idx1, sl_to_idx idx2)
  | Some ([[{ it = Atom "ELEM.DROP"; _ }]; _], [idx]) -> ElemDrop (sl_to_idx idx)
  | Some ([[{ it = Atom "LOAD"; _ }]; _; _], [idx; op]) -> Load (sl_to_idx idx, sl_to_loadop op)
  | Some ([[{ it = Atom "STORE"; _ }]; _; _], [idx; op]) -> Store (sl_to_idx idx, sl_to_storeop op)
  | Some ([[{ it = Atom "VEC.LOAD"; _ }]; _; _], [idx; op]) -> VecLoad (sl_to_idx idx, sl_to_vec_loadop op)
  | Some ([[{ it = Atom "VEC.STORE"; _ }]; _; _], [idx; op]) -> VecStore (sl_to_idx idx, sl_to_vec_storeop op)
  | Some ([[{ it = Atom "VEC.LOAD_LANE"; _ }]; _; _; _], [idx; op; i]) -> VecLoadLane (sl_to_idx idx, sl_to_vec_laneop op, sl_to_int i)
  | Some ([[{ it = Atom "VEC.STORE_LANE"; _ }]; _; _; _], [idx; op; i]) -> VecStoreLane (sl_to_idx idx, sl_to_vec_laneop op, sl_to_int i)
  ...
  | Some ([[{ it = Atom "MEMORY.COPY"; _ }]; _; _], [idx1; idx2]) -> MemoryCopy (sl_to_idx idx1, sl_to_idx idx2)
  | Some ([[{ it = Atom "MEMORY.INIT"; _ }]; _; _], [idx1; idx2]) -> MemoryInit (sl_to_idx idx1, sl_to_idx idx2)
  ...
  | Some ([[{ it = Atom "STRUCT.NEW"; _ }]; _; _], [idx; initop]) -> StructNew (sl_to_idx idx, sl_to_initop initop)
  | Some ([[{ it = Atom "STRUCT.GET"; _ }]; _; _; _], [idx1; idx2; ext_opt]) -> StructGet (sl_to_idx idx1, sl_to_idx idx2, sl_to_opt sl_to_extension ext_opt)
  | Some ([[{ it = Atom "STRUCT.SET"; _ }]; _; _], [idx1; idx2]) -> StructSet (sl_to_idx idx1, sl_to_idx idx2)
  | Some ([[{ it = Atom "ARRAY.NEW"; _ }]; _; _], [idx; initop]) -> ArrayNew (sl_to_idx idx, sl_to_initop initop)
  | Some ([[{ it = Atom "ARRAY.NEW_FIXED"; _ }]; _; _], [idx; n]) -> ArrayNewFixed (sl_to_idx idx, sl_to_nat32 n)
  | Some ([[{ it = Atom "ARRAY.NEW_ELEM"; _ }]; _; _], [idx1; idx2]) -> ArrayNewElem (sl_to_idx idx1, sl_to_idx idx2)
  | Some ([[{ it = Atom "ARRAY.NEW_DATA"; _ }]; _; _], [idx1; idx2]) -> ArrayNewData (sl_to_idx idx1, sl_to_idx idx2)
  | Some ([[{ it = Atom "ARRAY.GET"; _ }]; _; _], [idx; ext_opt]) -> ArrayGet (sl_to_idx idx, sl_to_opt sl_to_extension ext_opt)
  | Some ([[{ it = Atom "ARRAY.SET"; _ }]; _], [idx]) -> ArraySet (sl_to_idx idx)
  | Some ([[{ it = Atom "ARRAY.LEN"; _ }]], []) -> ArrayLen
  | Some ([[{ it = Atom "ARRAY.COPY"; _ }]; _; _], [idx1; idx2]) -> ArrayCopy (sl_to_idx idx1, sl_to_idx idx2)
  | Some ([[{ it = Atom "ARRAY.FILL"; _ }]; _], [idx]) -> ArrayFill (sl_to_idx idx)
  | Some ([[{ it = Atom "ARRAY.INIT_DATA"; _ }]; _; _], [idx1; idx2]) -> ArrayInitData (sl_to_idx idx1, sl_to_idx idx2)
  | Some ([[{ it = Atom "ARRAY.INIT_ELEM"; _ }]; _; _], [idx1; idx2]) -> ArrayInitElem (sl_to_idx idx1, sl_to_idx idx2)
...
and sl_to_global_type (value: Value.t) : Types.global_type =
  match sl_case value with
  | Some ([[{ it = Atom "GlobalT"; _ }]; _; _], [mut; vt]) ->
...
and sl_to_table_type (value: Value.t) : Types.table_type =
  match sl_case value with
  | Some ([[{ it = Atom "TableT"; _ }]; _; _; _], [at; limits; rt]) -> TableT (sl_to_addr_type at, sl_to_limits limits, sl_to_ref_type rt)
  | _ -> failwith "Expected tabletype"

and sl_to_memory_type (value: Value.t) : Types.memory_type =
  match sl_case value with
  | Some ([[{ it = Atom "MemoryT"; _ }]; _; _], [at; limits]) -> MemoryT (sl_to_addr_type at, sl_to_limits limits)
